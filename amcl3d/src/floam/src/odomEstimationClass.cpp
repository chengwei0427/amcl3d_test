...
void OdomEstimationClass::initMapWithPoints(const pcl::PointCloud<pcl::PointXYZI>::Ptr& edge_in, const pcl::PointCloud<pcl::PointXYZI>::Ptr& surf_in){
    ...
    *laserCloudSurfMap += *surf_in;
    pointCloudCornerQueue.push(*edge_in);
    pointCloudSurfQueue.push(*surf_in);
...
void OdomEstimationClass::addPointsToMap(const pcl::PointCloud<pcl::PointXYZI>::Ptr& downsampledEdgeCloud, const pcl::PointCloud<pcl::PointXYZI>::Ptr& downsampledSurfCloud){
    //储存点
    pcl::PointCloud<pcl::PointXYZI>::Ptr tempEdgeCloud(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::PointCloud<pcl::PointXYZI>::Ptr tempSurfCloud(new pcl::PointCloud<pcl::PointXYZI>);
    for (int i = 0; i < (int)downsampledEdgeCloud->points.size(); i++)
    {
        ...
        tempEdgeCloud->push_back(point_temp);
    }
    ...
    for (int i = 0; i < (int)downsampledSurfCloud->points.size(); i++)
    {
        ...
        tempSurfCloud->push_back(point_temp);
    }
    if(pointCloudCornerQueue.size()>queueSize)
    {
        pointCloudCornerQueue.pop();
        pointCloudSurfQueue.pop();
    }
    pointCloudSurfQueue.push(*tempSurfCloud);
    pointCloudCornerQueue.push(*tempEdgeCloud);
    ...
    {
        int qsize = pointCloudCornerQueue.size();
        for(int i = 0;i < qsize;i++)
        {
            *laserCloudCornerMap += pointCloudCornerQueue.front();
            *laserCloudSurfMap += pointCloudSurfQueue.front();
            pointCloudCornerQueue.push(pointCloudCornerQueue.front());
            pointCloudSurfQueue.push(pointCloudSurfQueue.front());
            pointCloudCornerQueue.pop();
            pointCloudSurfQueue.pop();
        }
    }   
